...
class SecuritePage extends ConsumerWidget {
  ...
  void _showChangePasswordDialog(BuildContext context, WidgetRef ref) {
    ...
    AppDialogs.showConfirmDialog(
      ...
      message: 'Un email de réinitialisation sera envoyé à $email. Voulez-vous continuer ?',
      confirmText: 'Envoyer',
      cancelText: 'Annuler',
    ).then((confirmed) async {
      if (confirmed == true && context.mounted) {
        final success = await ref.read(authProvider.notifier).resetPassword(email: email);
        if (context.mounted) {
          if (success) {
...
class _ShieldPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(Rect.fromLTWH(w * 0.5, h * 0.5, w * 0.5, h * 0.5), pBottomRight);
